...
L<regexp|/"regexp"> or L<index|/"index"> are used to specify the image, then
L<search_url|/"search_url"> must be set to specify the page to be searched for the
image.

B<Image::Grab> will use the data in the following order: L<url|/"url">,
L<regexp|/"regexp">, L<index|/"index">.

L<refer|/"refer">, L<regexp|/"regexp">, L<search_url|/"search_url">
and L<url|/"url"> all have POSIX time string expansion performed on
them by the L<expand_url|/"expand_url_tries_"> method when
L<do_posix|/"do_posix"> is set.  Thus, if you wish to have a '%'
...
POSIX time string expansion is performed if L<do_posix|/"do_posix"> is set.
...
If L<regexp|/"regexp"> and/or L<index|/"index"> methods are used to specify an image then
...
L<search_url|/"search_url">.  Used in conjunction with L<regexp|/"regexp">, it specifies which
...
L<index|/"index"> is not set, then the first image that matches will be used.
If L<index|/"index"> is set, then the I<n>th image (base 0) that matches will
be used.

Set L<search_url|/"search_url"> to the web page that you want to search with this
regular expression.
...
If the L<url|/"url"> method is not used to give an absolute
...
If neither L<date|/"date"> nor L<md5|/"md5> are set, than this method acts
identically to L<grab|/"grab_tries_">.  

If L<md5|/"md5"> is set, then the grab is performed only if the checksum of
the newer image is different than the current checksum.

If L<date|/"date"> is set than the grab is performed only if the image has
been modified since L<date|/"date">.

If both L<date|/"date"> and L<md5|/"md5"> are set then the conditions are ANDed.
That is, the image is returned only if it has been modified since
L<date|/"date"> and its checksum is different than L<md5|/"md5">.
...
These are miscellaneous properties.  L<do_posix|/"do_posix"> and
L<cookiefile|/"cookiefile"> are the only ones you should need to use.
...
L<search_url|/"search_url">.  Of course, L<search_url|/"search_url"> must be set for this method
to be of any use.
...
L<search_url|/"search_url"> URL is not of type text/html, or if no image that
matches the specs is found.

If L<url|/"url"> is given a full URL, then it is returned with POSIX time
string expansion performed if L<do_posix|/"do_posix"> is set.
